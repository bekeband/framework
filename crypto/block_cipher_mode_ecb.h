 ...
 *
 *********************************************************************
 * FileName:        block_cipher_mode_ecb.h
 * Dependencies:    None
...
typedef struct
{
    uint8_t __attribute__((aligned)) remainingData[CRYPTO_CONFIG_BLOCK_MAX_SIZE];     // Buffer to store data until more is avaliable if there is not enough to encrypt an entire block.
...
/* Function:
  ...
  Description:
    Initializes a ECB context for encryption/decryption.  The user will specify 
    details about the algorithm being used in ECB mode.
...
/**************************************************************************************************************************************************************************************
  ...
  Description:
    Encrypts plain text using electronic codebook mode.
  Conditions:
    The ECB context must be initialized with the block cipher
  ...
  Input:
    ...
    numPlainBytes -   The number of plain text bytes that must be encrypted.
                      ...
                      bytes will be cached in the ECB context structure
  ...
  Example:
    ...
    //Encrypt the data
    BLOCK_CIPHER_ECB_Encrypt (handle, cipher_text, &num_bytes_encrypted, (void *) plain_text, sizeof(plain_text), &round_keys, &context, BLOCK_CIPHER_OPTION_STREAM_START);
    </code>
...
/* Function:
  ...
  Example:
    ...
    // Decrypt the data
    BLOCK_CIPHER_ECB_Decrypt (handle, plain_text, &num_bytes_decrypted, (void *) cipher_text, sizeof(cipher_text), &round_keys, &context, BLOCK_CIPHER_OPTION_STREAM_START);
    </code>
*/
