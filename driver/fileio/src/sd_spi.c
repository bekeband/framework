...
bool FILEIO_SD_MediaDetect (FILEIO_SD_DRIVE_CONFIG * config)
{
	...
	if (SPIENABLE == 0)
	{
        ...
        do
        {
            ...
            SD_CS = 1;
            FILEIO_SD_SPI_Put_Slow(0xFF);   //Send some "extraneous" clock pulses.  If a previous
            ...
            response = FILEIO_SD_SendMediaCmd_Slow(GO_IDLE_STATE, 0x0);
        } while((response.r1._byte != 0x01) && (timeout != 0));

	    //Check if response was invalid (R1 response uint8_t should be = 0x01 after GO_IDLE_STATE)
	    ...
	    else
	    {
    		...
    		FILEIO_SD_MediaInitialize();    //Can block and take a long time to execute.
	...
	else
	{
    	...
    	response = FILEIO_SD_SendCmd(SEND_STATUS,0x0);
...
FILEIO_SD_RESPONSE FILEIO_SD_SendCmdSlow(FILEIO_SD_DRIVE_CONFIG * config, uint8_t cmd, uint32_t address)
{
    ...
    //Special handling for CMD12 (FILEIO_SD_STOP_TRANSMISSION).  The very first uint8_t after
    //sending the command packet may contain bogus non-0xFF data.  This 
    //"residual data" uint8_t should not be interpreted as the R1 response uint8_t.
    if(cmd == FILEIO_SD_STOP_TRANSMISSION)
    {
        FILEIO_SD_SPI_Get_Slow(config->index); //Perform dummy read to fetch the residual non R1 uint8_t
    } 
    ...
    {
        response.r2._byte1 = response.r1._byte; //We already received the first uint8_t, just make sure it is in the correct location in the struct.
        response.r2._byte0 = FILEIO_SD_SPI_Get_Slow(config->index); //Fetch the second uint8_t of the response.
    }
    else if(sdmmc_cmdtable[cmd].responsetype == FILEIO_SD_RESPONSE_R1b)
    {
        //Keep trying to read from the media, until it signals it is no longer
...
/*****************************************************************************
  ...
  Input:
    FILEIO_SD_ASYNC_IO* info -        A pointer to a FILEIO_SD_ASYNC_IO structure.  The 
                            ...
                            the read operation (ex: number of total uint8_ts to read,
                            where to copy them once read, maximum number of uint8_ts
  ...
  Return Values:
    uint8_t - Returns a status uint8_t indicating the current state of the read 
            ...
            FILEIO_SD_ASYNC_READ_COMPLETE - Returned when all data uint8_ts in the read 
  ...
  Description:
    ...
    the block size (theoretically anything up to ~4GB, since uint8_tsRemaining is 
    ...
    of uint8_ts have been read.  This puts the media back into the default state 
    ...
    2. FILEIO_SD_ASYNC_READ_NEW_PACKET_READY - repeatedly, until 512 uint8_ts [media read 
        block size] is received, then 
    3. Back to FILEIO_SD_ASYNC_READ_BUSY (for awhile, may be short), then
    4. Back to FILEIO_SD_ASYNC_READ_NEW_PACKET_READY (repeatedly, until the next 512 uint8_t
  ...
  Remarks:
    ...
    info->numuint8_ts and info->pReceiveBuffer are allowed to change however.
    
    The uint8_tsRemaining value must always be an exact integer multiple of numuint8_ts 
    for the function to operate correctly.  Additionally, it is recommended, although
    not essential, for the uint8_tsRemaining to be an integer multiple of the media
    read block size.
...
uint8_t FILEIO_SD_AsyncReadTasks(FILEIO_SD_DRIVE_CONFIG * config, FILEIO_SD_ASYNC_IO* info)
{
    ...
    switch(info->bStateVariable)
    {
        ...
        case FILEIO_SD_ASYNC_READ_QUEUED:
            ...
            //SDHC cards are addressed on a 512 uint8_t block basis.  This is 1:1 equivalent
        ...
        case FILEIO_SD_ASYNC_READ_WAIT_START_TOKEN:
            ...
            //us the data start token uint8_t.  This could typically take a 
...
uint8_t FILEIO_SD_AsyncWriteTasks(FILEIO_SD_DRIVE_CONFIG * config, FILEIO_SD_ASYNC_IO* info)
{
    ...
    switch(info->bStateVariable)
    {
        ...
        case FILEIO_SD_ASYNC_WRITE_TRANSMIT_PACKET:
            ...
            //Check if we have finshed sending a 512 uint8_t block.  If so,
...
FILEIO_MEDIA_INFORMATION *  FILEIO_SD_MediaInitialize (FILEIO_SD_DRIVE_CONFIG * config)
{
    ...
    //If it does support the range, it will respond with a type R7 reponse packet (6 uint8_ts/48 bits).	        
    ...
    if(((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA) && (!response.r7.bitwise.bits.ILLEGAL_CMD))
   	{
		...
		//Send CMD58 (Read OCR [operating conditions register]).  Reponse type is R3, which has 5 uint8_ts.
    ...
    else
	{
        ...
        else
        {
            ...
            //Set read/write block length to 512 uint8_ts.  Note: commented out since
            //this theoretically isn't necessary, since all cards v1 and v2 are 
            //required to support 512 uint8_t block size, and this is supposed to be
...
//    		OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2);
	...
	//READ_BL_LEN = 1024 or 2048 uint8_ts (and therefore WRITE_BL_LEN also 
