...
;  WREG7 - psv table offset
    .global    _AES_RoundKeysCreate128
_AES_RoundKeysCreate128:
.ifndecl __HAS_EDS
    push CORCON
.endif
    mov        PSVPAG, W3
    ...
    mov     #10,W1              ;Load loop counter - 10 iterations

_AES_RoundKeysCreate128_Loop:
    ;loop
    clr     W2                  ;clear W2 so that upper byte doesn't affect offset

    ;w[i] = SubWord(w[i-1])^w[i-Nk];
    mov.b    [W0-3], W2            ;RotWord(w[i-1]) for byte 3
    mov.b    [W7+W2], W2         ;SubWord(w[i-1]) for byte 3
    xor.b    W2, W5, W2            ;^rcon
    xor.b    W2, [W6++], [W0++]  ;^w[i-Nk]
    mov.b    [W0-3], W2            ;RotWord(w[i-1]) for byte 2
    mov.b    [W7+W2], W2         ;SubWord(w[i-1]) for byte 2
    xor.b    W2, [W6++], [W0++]  ;^w[i-Nk]
    mov.b    [W0-3], W2            ;RotWord(w[i-1]) for byte 1
    mov.b    [W7+W2], W2         ;SubWord(w[i-1]) for byte 1
    xor.b    W2, [W6++], [W0++]  ;^w[i-Nk]
    mov.b    [W0-7], W2            ;RotWord(w[i-1]) for byte 0
    mov.b    [W7+W2], W2         ;SubWord(w[i-1]) for byte 0
    xor.b    W2, [W6++], [W0++]  ;^w[i-Nk]
    ...
    dec        W1, W1
    bra        NZ, _AES_RoundKeysCreate128_Loop
