/*******************************************************************************
  ...
  Summary:
    RSA driver source code

  Description:
    This file contains the source code that implements the interface the RSA 
...
#include "crypto/src/rsa/other/rsa_helper.h"
...
*/
int DRV_RSA_Configure(DRV_HANDLE handle, uint8_t *xBuffer, uint8_t *yBuffer, uint16_t xLen, uint16_t yLen, DRV_RSA_RandomGet randFunc, DRV_RSA_PAD_TYPE padType)
{
    if (handle != DRV_RSA_HANDLE || padType != DRV_RSA_PAD_DEFAULT)
    {
        return -1;
    }
...
DRV_RSA_STATUS DRV_RSA_Encrypt (DRV_HANDLE handle, uint8_t *cipherText, uint8_t *plainText, uint16_t msgLen, const DRV_RSA_PUBLIC_KEY *publicKey)
{
    ...
    if(rsaDesc0.op != DRV_RSA_OPERATION_MODE_NONE)
    {
        return DRV_RSA_STATUS_ERROR;
    }    

    if (handle != DRV_RSA_HANDLE)
    {
    ...
    rsaDesc0.op = DRV_RSA_OPERATION_MODE_ENCRYPT;
    rsaDesc0.status = DRV_RSA_STATUS_BUSY;
...
DRV_RSA_STATUS DRV_RSA_Decrypt (DRV_HANDLE handle, uint8_t *plainText, uint8_t *cipherText, uint16_t * msgLen, const DRV_RSA_PRIVATE_KEY_CRT *privateKey)
{
    ...
    rsaDesc0.status = DRV_RSA_STATUS_BUSY;
    rsaDesc0.op = DRV_RSA_OPERATION_MODE_DECRYPT;
    ...
    if ((*endPtr-- == 0x00) && (*endPtr-- == 0x02))
    {
        ...
        {
            rsaDesc0.status = DRV_RSA_STATUS_ERROR;
        }
        else
        {
...
void DRV_RSA_Tasks(SYS_MODULE_OBJ object)
{
    ...
    if (rsaDesc0.status == DRV_RSA_STATUS_BUSY)
    {
        if (rsaDesc0.op == DRV_RSA_OPERATION_MODE_ENCRYPT)
        {
            chk_done = RSAModExp(&rsaDesc0.cT, &rsaDesc0.pT, &rsaDesc0.pkExp, &rsaDesc0.pkN);
            if (chk_done)
            {
        ...
        else if (rsaDesc0.op == DRV_RSA_OPERATION_MODE_DECRYPT)
        {
            tmpPtr = &tmp;
            tmpPtr->startAddress = rsaDesc0.xBuffer;
            tmpPtr->bLength = rsaDesc0.length * 2;
...
void RSAPadData (uint8_t * result, uint8_t * source, uint16_t msgLen, uint16_t keyLen, RSA_RandomGet randFunction, DRV_RSA_PAD_TYPE padType)
{
    ...
    switch (padType)
    {
        case DRV_RSA_PAD_DEFAULT:
        case DRV_RSA_PAD_PKCS1:
