...
/*************************************************************************** 
 Overview: This is a Meter compile time option to define how many characters 
           ...
           Example: 
           if maxValue-minValue = 500, GFX_CONFIG_OBJECT_METER_SCALE_COUNT should be 3. 
...
typedef struct
{
    ...
    // they can be different from the scheme font.
    // Note that the sizes of these fonts are not checked with the meter dimension. 
    ...
    GFX_RESOURCE_HDR    *pValueFont;    // Pointer to the font used in the 
                                        // current reading (if displayed) of the meter
} GFX_GOL_METER;
