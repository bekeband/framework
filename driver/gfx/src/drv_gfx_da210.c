...
// *****************************************************************************
    ...
    extern uint8_t PaletteBpp;
    extern uint8_t blPaletteChangeError;
    extern void *pPendingPalette;
    extern uint16_t PendingStartEntry;
    extern uint16_t PendingLength;
#endif
...
static uint16_t GFX_FreeCommandSpaceCheck(uint16_t spaceNeeded);
                           ...
                           )
{
    ...
    for(y = 0; (y < bitmapHdr.height) && (yc <= GFX_MaxYGet()); y++)
    {
        ...
        pData = GFX_RenderingBufferGet() + (outputSize - 1); 
                                ...
                                GFX_PARTIAL_IMAGE_PARAM *pPartialImageData)
{
    ...
    for(y = 0; (y < bitmapHdr.height) && (yc <= GFX_MaxYGet()); y++)
    {
        ...
        if(IsPaletteEnabled())
        {
            pByteData = (uint8_t*)(GFX_RenderingBufferGet() + (outputSize - 1));
...
#ifndef GFX_CONFIG_DOUBLE_BUFFERING_DISABLE
// TODO: confirm if this code is needed
//    // synchronize the draw and frame buffer
...
/*********************************************************************
* Function: uint8_t IsPaletteEnabled(void)
*
...
*
********************************************************************/
uint8_t IsPaletteEnabled(void)
{
    return _CLUTEN;
}

/*********************************************************************
* Function: uint8_t SetPaletteBpp(uint8_t bpp)
*
...
*
********************************************************************/
uint8_t SetPaletteBpp(uint8_t bpp)
{
    switch(bpp)
    {
...
/*********************************************************************
* Function: uint8_t SetPaletteFlash(const PALETTE_ENTRY *pPaletteEntry, uint16_t startEntry, uint16_t length)
*
...
*
********************************************************************/
uint8_t SetPaletteFlash(const GFX_PALETTE_ENTRY *pPaletteEntry, uint16_t startEntry, uint16_t length)
{
    uint16_t counter;

    if(pPaletteEntry == NULL)
        return 1;
    else if ((startEntry + length) > 256)
        return 2;
